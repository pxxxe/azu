...
class LayerLoader:
    ...
    async def _load_weights_safe(self, path):
        ...
        Critical for preventing the P2P heartbeat from dying while loading 2GB files.
        """
                ...
                async with session.get(url) as resp:
                    ...
                    with open(temp, 'wb') as f:
                        ...
                        async for chunk in resp.content.iter_chunked(1024 * 1024): # 1MB chunks
                            ...
                            # CRITICAL FIX: Yield to event loop during heavy disk writes.
                            # Keeps P2P server responsive.
    ...
    async def load_dense_layer(self, model_id: str, layer_idx: int, device="cuda"):
        ...
        print(f"   📥 torch.load({path.name})... (Threaded)")
        sys.stdout.flush()
